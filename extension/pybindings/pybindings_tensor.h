...
/** A small Python-facing CPU tensor with no ATen dependency. */
 ...
 public:
  explicit PyTensor(
    ...
    py::object array_object = dtype.is_none()
        ? numpy.attr("asarray")(data)
        : numpy.attr("asarray")(data, py::arg("dtype") = dtype);
    ...
    initialize_contiguous_metadata();
    data_.resize(array.nbytes());
    if (!data_.empty()) {
      std::memcpy(data_.data(), array.data(), data_.size());
    }
    validate_nbytes();
  }
 ...
 private:
  ...
  PyTensor(
    ...
    if (nbytes > 0) {
      ...
      std::memcpy(data_.data(), data, nbytes);
    }
    if (strides_.empty()) {
      initialize_contiguous_metadata();
    }
    if (dim_order_.empty()) {
      initialize_dim_order();
    }
  ...
  void initialize_contiguous_metadata() {
    strides_.resize(sizes_.size());
    int32_t stride = 1;
    for (size_t i = sizes_.size(); i > 0; --i) {
      strides_[i - 1] = stride;
      const auto size = sizes_[i - 1];
      if (size != 0 && stride > std::numeric_limits<int32_t>::max() / size) {
