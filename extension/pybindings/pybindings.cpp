...
struct PyMethod final {
  ...
  void set_inputs(const py::object& python_inputs) {
    ...
    std::vector<EValue> cpp_inputs;
    cpp_inputs.reserve(inputs_size);
    portable_inputs_.clear();
    portable_inputs_.reserve(inputs_size);
    portable_tensor_ptrs_.clear();
    portable_tensor_ptrs_.reserve(inputs_size);
#ifdef EXECUTORCH_PYBIND_USE_ATEN
    use_portable_tensors_ = false;
#else
    use_portable_tensors_ = true;
#endif
    ...
    for (size_t i = 0; i < inputs_size; ++i) {
      ...
      if (py::isinstance<PyTensor>(python_input)) {
        ...
        cpp_inputs.emplace_back(portable_tensor_ptrs_.back());
#endif
#ifdef EXECUTORCH_PYBIND_USE_ATEN
      } else if (is_torch_tensor(python_input)) {
        auto at_tensor = python_input.cast<at::Tensor>();
